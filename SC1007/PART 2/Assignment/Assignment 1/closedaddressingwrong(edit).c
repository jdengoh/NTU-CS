...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    //printf("%d", Q1Ptr->hSize);

    if (Q1Ptr->Table[i].head == NULL) {

        Q1Ptr->Table[i].head = malloc(sizeof(ListNode));

        (Q1Ptr->Table[i].head)->key = key;
        (Q1Ptr->Table[i].head)->next = NULL;
        (Q1Ptr->Table[i].head)->pre = NULL;


        Q1Ptr->Table[i].size = (Q1Ptr->Table[i].size + 1);
        Q1Ptr->nSize = Q1Ptr->nSize+1;
        return 1;
    }

    else {

        cur = Q1Ptr->Table[i].head;
        ...
        pre->key = key;
        pre->next = Q1Ptr->Table[i].head;
        Q1Ptr->Table[i].head->pre = pre;

        Q1Ptr->Table[i].head = pre;
