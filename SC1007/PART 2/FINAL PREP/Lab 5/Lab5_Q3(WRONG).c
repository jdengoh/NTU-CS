...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
 ...
 int hash1 = Hash(key, Q3Hash.hSize);
 int newhash;
 int count = 0;
 ListNode *temp;

    newhash = (hash1)%(Q3Hash.hSize);
    temp = Q3Hash.Table->head;
    ...
    while(temp!=NULL && count!=Q3Hash.hSize) {

        if (temp->key == key) return temp;

        temp=temp->next;
        newhash = (newhash+1)%Q3Hash.hSize;
        count++;

        if (temp == NULL && newhash == (Q3Hash.hSize-1)) {
            temp = Q3Hash.Table->head;
        }
...
{
 // Write your code here
 int hash1 = Hash(key, Q3HashPtr->hSize);
 int newhash;
 ListNode *temp;

 temp = Q3HashPtr.Table->head;

 if (HashSearch(*Q3HashPtr, key) != NULL) {
    return 0;
 }

 if (Q3HashPtr->hSize == Q3HashPtr->nSize) {
    return Q3HashPtr->hSize;
 }
