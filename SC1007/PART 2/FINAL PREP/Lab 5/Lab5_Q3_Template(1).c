...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
 ...
 int hash = Hash(key, Q3Hash.hSize);

 ListNode *temp;

 if (Q3Hash.Table[hash].size == 0) {
    return 0;
 }

 else {

    temp = Q3Hash.Table[hash].head;
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
 ...
 if(HashSearch(*Q3HashPtr, key) != NULL) return 0;

 else {
    temp = Q3HashPtr->Table[hash].head;

    Q3HashPtr->Table[hash].head = malloc(sizeof(ListNode));
    Q3HashPtr->Table[hash].head->key = key;
    Q3HashPtr->Table[hash].head->next = temp;
