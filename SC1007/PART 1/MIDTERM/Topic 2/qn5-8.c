...
int palindrome(char *word){
	...
	for (int i=0; i<size/2; i++) {
        push(&temp2, dequeue(&temp1));
	}

	if (size%2)
        dequeue(&temp1);

	printList(temp1.ll.head);
	printf("\n");
	printList(temp2.ll.head);

	while(!isEmptyStack(&temp2)) {
        if(pop(&temp2)!=dequeue(&temp1)) {
...
int balanced(char *expression){
	...
	while (*expression) {
        enqueue(&temp1,*expression);
        push(&temp2,*expression);
        expression++;
        size++;
	}
