...
int main()
{
	...
	for (i = 17; i > 10; i--)			//build linked list 2
...
int duplicateReverse(ListNode *head, ListNode **ptrNewHead){
    ...
    for (int i = size-1; i>=0; i--) {
        val = (findNode(head, i))->num;
        insertNode(ptrNewHead, ind, val);
        ind++;
    }
    return 0;
...
int concatenate(ListNode **ptrHead1, ListNode *head2){
	...
	while (head2!=NULL) {
        insertNode(ptrHead1, size, (head2->num));
        head2 = head2->next;
        size++;
	}
}
...
int combineAlternating(ListNode **ptrHead, ListNode *head1, ListNode *head2){
	...
	while (head1!=NULL && head2!=NULL) {
        if (count%2 == 0) {
            insertNode(ptrHead, count, head1->num);
            head1 = head1->next;
        }
        else {
            insertNode(ptrHead, count, head2->num);
            head2 = head2->next;
        }
        count++;
	}
